...
/* ---------- App (une seule fonction exportée) ---------- */
export default function SympaLanding() {
  // URLs d’assets compatibles GitHub Pages (servis depuis /public)
  const BASE_URL = process.env.PUBLIC_URL || "";
  const LOGO = `${BASE_URL}/logo-sympa.png`;
  const VIDEO_SRC = `${BASE_URL}/veo-demo.mp4`;
  const POSTER = `${BASE_URL}/poster-veo.jpg`; // optionnel si tu ajoutes ce fichier
    ...
    >
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-800 bg-slate-900/95">
            <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col gap-3">
              {(["features","video","pricing","events","faq"] as const).map((id) => (
                <a
                  key={id}
                  href={`#${id}`}
                  onClick={() => setMenuOpen(false)}
                  className={navLinkCls(id)}
                >
                  {id === "features" ? "Fonctionnalités"
                    : id === "video" ? "Vidéo"
                    : id === "pricing" ? "Offres"
                    : id === "events" ? "Événements"
                    : "FAQ"}
                </a>
              ))}
              <Button
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 items-center px-4 py-16">
          <div>
            ...
            <h1 className="mt-4 text-4xl md:text-5xl font-bold leading-tight">
              Le compagnon qui <span className="text-indigo-400">veille</span>,<br className="hidden sm:block" />
              écoute et protège
            </h1>
            ...
            <ul className="mt-6 space-y-2 text-sm text-slate-300">
              <li className="flex items-start gap-2"><Check className="h-5 w-5 text-indigo-400" /> Assistance au relevage (guidage et support)</li>
              <li className="flex items-start gap-2"><Check className="h-5 w-5 text-indigo-400" /> Interaction naturelle (voix, gestes, routines)</li>
              <li className="flex items-start gap-2"><Check className="h-5 w-5 text-indigo-400" /> Apparence <b>personnalisable</b> : chien ou chat</li>
            </ul>
          </div>
      ...
      <section id="features" className="py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="mt-8 grid md:grid-cols-4 gap-6">
            {[
              { src: "https://picsum.photos/seed/robotdog/800/600", txt: "Chien robot – configuré (couleurs, yeux LED, accessoires)." },
              { src: "https://picsum.photos/seed/robotcat/800/600", txt: "Chat robot – configuré (mouvements, sons, motifs)." },
              { src: "https://picsum.photos/seed/doggrandma/800/600", txt: "Assistance en promenade : vigilance & présence rassurante." },
              { src: "https://picsum.photos/seed/catbed/800/600", txt: "Veille de nuit : proximité discrète, alerte si besoin." },
            ].map((c, i) => (
              <Card key={i} className="overflow-hidden group">
                <img src={c.src} alt="" className="w-full h-44 object-cover transition-transform duration-300 group-hover:scale-105" />
      ...
      <section id="video" className="py-16 border-y border-slate-800">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <ul className="mt-4 space-y-2 text-sm text-slate-300">
              <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Versions « chien » et « chat »</li>
              <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Slogan : <i>« SYMPA, le compagnon qui veille et protège »</i></li>
            </ul>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 mt-3">
          <a href={VIDEO_SRC} download className="inline-flex items-center gap-2 text-sm text-indigo-400 hover:text-indigo-300 underline">
            ...
            <Card className="border-indigo-500/20">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <span className="bg-indigo-700/40 text-indigo-200 text-xs px-2 py-1 rounded-full ml-2">Garantie 5 ans</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm text-slate-300">
                <div className="text-3xl font-bold">5 000 €</div>
                <ul className="space-y-2">
                  <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Contrat de maintenance <b>5 ans</b> inclus</li>
                  <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Installation & paramétrage</li>
                </ul>
                <Button className="rounded-2xl w-full" onClick={() => setShowContact(true)}>Obtenir une offre</Button>
              </CardContent>
            </Card>
            ...
            <Card className="border-indigo-500/20">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <span className="bg-emerald-700/30 text-emerald-200 text-xs px-2 py-1 rounded-full ml-2">Souple</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm text-slate-300">
                <div className="text-3xl font-bold">50 € / mois</div>
                <ul className="space-y-2">
                  <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Robot inclus (chien ou chat)</li>
                  <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Mises à jour & télésurveillance</li>
                  <li className="flex gap-2"><Check className="h-5 w-5 text-indigo-400" /> Échange standard en 48h</li>
                </ul>
                <Button variant="outline" className="rounded-2xl w-full" onClick={() => setShowContact(true)}>Essayer 3 mois</Button>
              </CardContent>
            </Card>
          </div>
        </div>
      ...
      <section id="events" className="py-16 border-y border-slate-800">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="text-xs rounded-full px-3 py-1 bg-indigo-700/40 text-indigo-200 border border-indigo-500/30">Nouveau</span>
          </div>
            ...
            <div className="p-6 md:p-8 grid md:grid-cols-3 gap-6 md:gap-10 items-center">
              <div className="md:col-span-2">
                ...
                <h3 className="mt-3 text-2xl font-semibold">
                  Parmi les <span className="text-indigo-300">100 premiers robots achetés</span>, une chance de <span className="text-emerald-300">gagner 1 000 €</span> 🎉
                </h3>
                <p className="mt-2 text-slate-300 text-sm">
                  Tirage au sort parmi les 100 premières commandes (1 gagnant). Règlement disponible sur demande.
                </p>
                <div className="mt-4 flex flex-wrap gap-3">
                  <Button className="rounded-2xl" onClick={() => setShowContact(true)}>Je participe</Button>
                  <Button variant="outline" className="rounded-2xl" onClick={() => window.location.assign("#pricing")}>Voir les offres</Button>
                </div>
              </div>
              ...
              <div className="md:justify-self-end w-full">
                <div className="rounded-2xl border border-slate-700 bg-slate-900 p-4">
                  ...
                  <div className="mt-4 text-xs text-slate-400">* Exemple statique pour la démo (non connecté).</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8">
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 text-slate-300">
              ...
              <p className="text-sm mt-1">Démo, journées portes ouvertes et ateliers aidants. Restez connectés !</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="faq" className="py-16 border-y border-slate-800">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-6 text-sm text-slate-300">
          <Card>
            ...
            <CardContent>Oui, SYMPA suit un plan sécurité et des tests de qualification. Les aides au relevage sont guidées.</CardContent>
          </Card>
          <Card>
